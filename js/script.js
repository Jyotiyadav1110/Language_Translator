...
exchangeIcon.addEventListener('click',()=>{
    //excahnge textarea and sleect tag values
...
//fetching API response and returning it with parsing it with parsing into js obj and in naother then mehtod receiving that obj
        fetch(`https://api.mymemory.translated.net/get?q=${text}&langpair=${translateFrom}|${translateTo}`).then(res=>res.json()).then(data =>{
            console.log(data);
...
icons.forEach(icon =>{
    icon.addEventListener('click', ({target})=>{
        if(target.classList.contains("fa-copy")){
            //if clicked icon has from id, copy the fromTextarea value else the toTextarea value
            if(target.id == "from"){
